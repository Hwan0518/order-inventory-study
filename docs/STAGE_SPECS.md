...
통과 기준 항목 자체를 서술하지 않는다.

#### Q1

질문: `재고를 증가하거나 차감하는 도메인 메서드가 항상 지켜야 할 조건은 무엇인가요?`

상황: `재고를 변경하는 흐름은 지금 설계하는 주문 처리와 입고 처리, 이 두
가지뿐입니다. 요청은 항상 하나씩 순서대로 들어와 처리됩니다.`
...
상황: `상품을 판매하는 곳은 이 시스템 하나뿐이고, 주문이 들어오면 결제는
항상 성공한다고 가정합니다. 재고와 주문 정보는 모두 이 애플리케이션의
데이터베이스에만 저장됩니다.`
...
상황: `재고, 주문, 주문 항목은 모두 같은 애플리케이션 코드 안에서, 같은
데이터베이스의 테이블로 관리됩니다. 이 흐름 중간에 외부 서비스나 메시지 큐를
거치는 단계는 없습니다.`
...
상황: `재고를 변경하는 코드 경로는 지금 설계하는 주문 생성 흐름과 입고 흐름,
이 두 곳뿐입니다.`
...
다룰 개념(디코딩 금지, 저작 참고용): `7J6s6rOgIO2ZleyglSDssKjqsJAg7Iuc7KCQLCDsi6TtjKgg7IucIOuzteq1rCwg7KO866y4IOyDge2DnCDqtIDrpqw=`
...
배경: 유저 주문 vs 어드민 재고 조작 동시 접근.

다룰 개념(디코딩 금지, 저작 참고용): `TG9zdCBVcGRhdGUsIOu5hOq0gOyggS/rgpnqtIDsoIEg6529LCDrjbDrk5zrnb0sIOyerOyLnOuPhA==`

## 4단계 — 미정

배경: 유통 채널 추가 (단일 서버 유지).

다룰 개념(디코딩 금지, 저작 참고용): `7LGE64SQ67OEIOyerOqzoCDshozsnKAgdnMg6rO17JygIO2SgCwg7LSI6rO8IO2MkOunpCDrsKnsp4AsIOyxhOuEkCDqsIQg64+Z7Iuc7ISx`

## 5단계 — 미정

배경: 재고관리 전담 WMS 서버 분리.

다룰 개념(디코딩 금지, 저작 참고용): `67aE7IKwIO2KuOuenOyereyFmCDrtoDsnqwsIOuEpO2KuOybjO2BrCDsi6TtjKgv7YOA7J6E7JWE7JuDLCDrqbHrk7HshLEsIOy1nOyiheyggSDsnbzqtIDshLEsIOuztOyDgSDtirjrnpzsnq3shZg=`
